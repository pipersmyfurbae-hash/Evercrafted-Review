...
vi.mock("./cometClaude", async () => { const { invokeLLM: mockedInvokeLLM } = await import("./_core/llm"); return { generateClaudeJson: vi.fn(async (messages: Array<{ role: "system" | "user"; content: string }>) => mockedInvokeLLM({ messages } as never)), generateClaudeStory: vi.fn(async () => ({ title: "The Quiet Hour", body: "The house holds the last blue of morning while someone moves through its rooms with the care of a person learning to stay. The door remembers every season. At the table, stems wait in water, each one carrying a small decision. By afternoon, the circle is complete, and the threshold feels less like an entrance than a promise kept. The finished wreath gathers the room around it without asking for attention. Seven small changes of light carry the story forward, from the first hush to the hand that finally turns the latch.", metadata: { atmosphere: "late autumn", collectionName: "Quiet Hour", movement: "The After", silenceArc: [30, 120] }, grounding: { sourceDetails: ["A long memory of a quiet inherited room and the lake beyond it."], interpretations: ["The room carries a suspended belonging."], unsupportedClaims: [], majorUnsupportedClaims: [], approvalEligible: true }, designSignals: { emotions: ["belonging"], emotionalMovement: "toward the threshold", intensity: 4, atmosphere: "late autumn", sensoryEvidence: ["lake air"], symbolicThemes: ["threshold"], paletteDirection: ["aged ivory"], materialQualities: ["weathered"], directionalFlowCharacter: "toward the threshold", focalCharacter: "the remembered room", negativeSpaceMeaning: "breathing room", avoidances: ["invented biography"] }, beats: Array.from({ length: 7 }, (_, index) => ({ name: `Beat ${index + 1}`, role: "narrative moment", setting: "A quiet threshold", camera: "50mm standard", light: "soft dawn", prompt: "cinematic Evercrafted scene" })) })), storyGenesisProvider: { provider: "cometapi", model: "claude-sonnet-5", endpoint: "https://api.cometapi.com/v1/chat/completions" } }; });
...
describe("Signature Wreath router integration", () => {
  ...
  it("carries an approved profile through Story Genesis, florals, blueprint, and prompt compilation", async () => {
    ...
    db.select.mockImplementationOnce(() => selectRows([{ ...(persistedStories[0] ?? {}), id: 77, projectId: 1, emotionalProfileId: 2, status: "awaiting_approval" }]) as never);
